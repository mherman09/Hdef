...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -disp disp.tmp
...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -strain strain.tmp
cat > answer.tmp << EOF
...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -stress stress.tmp
cat > answer.tmp << EOF
...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -estress estress.tmp
cat > answer.tmp << EOF
...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -trg trg.tmp -normal normal.tmp
cat > answer.tmp << EOF
...
# Shear traction
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -trg trg.tmp -shear shear.tmp
cat > answer.tmp << EOF
...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -trg trg.tmp -coul coul.tmp
cat > answer.tmp << EOF
...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -trg trg.tmp -haf haf.tmp -coul coul.tmp
cat > answer.tmp << EOF
...
-5 55 -93 0.4
EOF
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -trg trg.tmp -coul coul.tmp -prog
cat > answer.tmp << EOF
  -123.00000000000000        40.200000000000003        0.0000000000000000       -2023107.5884089721
...
$BIN_DIR/triutil -flt tri.tmp -sta sta.tmp -disp disp.tmp -xy
