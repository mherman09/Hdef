...
# Usage statement
function usage() {
    echo "Usage: $0 -f SEIS_FILE -start DATE_START -end DATE_END -dday DDAY -Rw/e/s/n" 1>&2
    echo "       [-mag:min MAG_MIN] [-mag:max MAG_MAX] [-fade FADE_TIME] [-trans:max MAX_TRANS]" 1>&2
    echo "       [-mech]" 1>&2
    exit 1
}
...
NCOLORS=$(wc nday_color.tmp | awk '{print $1+1}')
colortool -hue -20,90 -lightness 30,95 -chroma 100,100 -ncolors $NCOLORS > color_list.tmp
# gmt makecpt -Cbatlow -T0/$NCOLORS/1 | awk '{if(!($1~/[A-N]/)){print $2}}' > color_list.tmp
awk '{
    if (NR==1) {
        print 0,$1
    } else {
    ...
    awk '{if(NF>1){print $1,$3,$2,$3}}END{print "B black";print "F",$1}' > time.cpt
...
do
    ...
    awk '{print $3,$4,$1,$5*$5*$5*0.001,$6}' seis_range.tmp |\
        gmt psxy $MAP_PROJ $MAP_LIMS -Sci -W0.5p -Ctime.cpt -t -K -O >> $PSFILE

    # Focal mechanisms
    if [ "$PLOT_MECH" == "Y" ]
    then
        awk '{if($7!=0||$8!=0||$9!=0){print $3,$4,$1,$7,$8,$9,$5*$5*$5*0.001,$6/1.2}}' seis_range.tmp |\
            gmt psmeca $MAP_PROJ $MAP_LIMS -Sa5i -L0.5p -Ctime.cpt -t -K -O >> $PSFILE
    fi
    ...
    gmt psxy -T -K -O -Y-2.0i >> $PSFILE

    # Magnitude versus time gridlines
    gmt psbasemap $TIME_PROJ $TIME_LIMS -Bsxg${YEAR_TIKS}Y -Byg1 -K -O --MAP_GRID_PEN=0.25p,225,4_2:0 >> $PSFILE

    # Seismicity
    awk '{print $2,$5,$1,$5*$5*$5*0.001,$6}' seis_range.tmp |\
        gmt psxy $TIME_PROJ $TIME_LIMS -Sci -W0.5p -Ctime.cpt -t -K -O >> $PSFILE

    # Focal mechanisms
    if [ "$PLOT_MECH" == "Y" ]
    then
        awk '{if($7!=0||$8!=0||$9!=0){print $2,$5,$1,$7,$8,$9,$5*$5*$5*0.001,$6/1.2}}' seis_range.tmp |\
            gmt psmeca $TIME_PROJ $TIME_LIMS -Sa5i -L0.5p -Ctime.cpt -t -K -O >> $PSFILE
    fi
...
gmt psconvert -Tg -A *.ps -V
rm *.ps

ffmpeg -framerate 24 -y -i "frame_%05d.png" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -vcodec libx264 -pix_fmt yuv420p mymovie.mp4
